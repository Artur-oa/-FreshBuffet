class UserValidator {
    ...
    static validatePassword(passwordHash) {
      ...
      if (!hasUpperCase.test(passwordHash) || !hasLowerCase.test(passwordHash) ||
          !hasNumbers.test(passwordHash) || !hasSpecialCharacters.test(passwordHash) ||
          !isValidLength
      ) {
        return false
      } else {
    ...
    static validate({ name, email, passwordHash }) {
      if (!name || !email || !passwordHash ||
          typeof name !== 'string' || typeof email !== 'string' || typeof passwordHash !== 'string' ||
          name.trim().length === 0 || email.trim().length === 0 || passwordHash.trim().length === 0
      ) {
